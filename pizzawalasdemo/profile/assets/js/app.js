...
async function onSignupFormSubmit() {
    ...
    if (isNameValid && isEmailValid && isPasswordValid && dob && gender && city && isZipcodeValid && state) {
        ...
        if (!city) {
            let errorMessage = "City cannot be empty. Please enter a valid city.";
...
function generateModal(title, message, type){
...
function triggerSuccessModal(){
    ...
    modal.innerHTML = generateModal("Welcome Aboard", "Congratulations on signing up for our loyalty program. Stay Tuned! Exciting offers and loyalty rewards are on your way.", "SUCCESS");
...
function triggerErrorModal(title, message){
    const backdrop = document.querySelector(".qb-modal-backdrop");
    const modal = document.querySelector(".qb-modal-container");
    modal.innerHTML = generateModal(title, message, "ERROR")
